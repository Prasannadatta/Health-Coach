...
recipe_df = recipe_df[['recipe_name', 'diet_labels', 'health_labels', 'cautions', 'cuisine_type', 'meal_type', 'dish_type', 'calorie_count', 'serving_size_gms', 'Total Fat_gms_per_serving', 'ingredients']]

nutrition_df = nutrition_df.fillna("").rename(columns=lambda c: str(c).strip())
nutrition_df = nutrition_df[['name', 'serving_size', 'calories', 'total_fat', 'protein']]
...
for i, r in recipe_df.iterrows():
    ...
    docs.append({"id": f"recipe_{i}", "type": "recipe", "instruction": f"{r.get('recipe_name','recipe')}", "input": "", "output": text})

# nutrition -> doc
for i, r in nutrition_df.iterrows():
    text = " | ".join([f"{col}: {r[col]}" for col in nutrition_df.columns if r[col] not in (None, "")])
    docs.append({"id": f"nut_{i}", "type": "ingredient", "instruction": f"{r.get('name','food')}", "input": "", "output": text})
